...
const HRhome = () => { 
    ...
    const fetchInterviewData = async () => {
      try {
        const response = await axios.get(
          `http://192.168.1.20:8081/api/v1/employee/contests/${id}`,
          {
            headers:{
        ...
        console.log("Contest IDs:", response);
  ...
  const handleClickContest = (contestId, contestName) => {
    navigate(`/hr/contestname/${contestId}`, { state: { interviewName: contestName } });
  };
      ...
      <div className="container-content">
        {interviewData.map((interview) => ( 
          <BgContainer
            ...
            path={`/hr/contestname/${interview.contestId}`} 
